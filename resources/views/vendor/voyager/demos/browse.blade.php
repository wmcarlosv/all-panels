                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            <div class="dropdown" id="menu-content" style="display: inline !important;">
                                              ...
                                              <ul class="dropdown-menu dropdown-menu-left" id="menu-list" aria-labelledby="dropdownMenu1">
                                                ...
                                                @endif
                                                <li><a href="#" class="convert-client" data-server="{{$data->server->name_and_local_name}}" data-row='{{json_encode($data)}}'>Convertir a Cliente</a></li>
                                              </ul>
                                            </div>
                                            @foreach($actions as $action)
                                                @if (!method_exists($action, 'massAction'))
                ...
                <form action="{{ route('convert_client') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="server" value="" readonly>
                        </div>
                        <div class="form-group">
                            <label for="">Paquete:</label>
                            <input type="text" class="form-control" id="package" value="" readonly>
                        </div>
                        <div class="form-group">
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            $("body").on('click','a.convert-client', function(){
                let row = JSON.parse($(this).attr("data-row"));
                $("input[name='demo']").val(row.id);
                $("#server").val(row.server.name+"("+row.server.local_name+")");
                $("#email").val(row.email);
                $("#password").val(row.password);
                $("#package").val(row.package ? row.package.name : '');
                $("#convert_customer").modal({keyboard: false, backdrop: 'static'}, 'show');
                return false;
            });
