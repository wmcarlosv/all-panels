...
class ServerController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $servers = Server::where('is_demo',0)->where('status', 1)->get();
    ...
    public function store(Request $request)
    {
        ...
        if($data){
            $updateServer = Server::find($data->id);
            $updateServer->last_token_update = date('Y-m-d H:i:s');
            $updateServer->json_data = json_encode($this->plex->serverData);
            $updateServer->update();
        }
