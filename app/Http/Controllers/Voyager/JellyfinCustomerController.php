...
class JellyfinCustomerController extends VoyagerBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if($data){
            $redirect = redirect()->back();
            $server = JellyfinServer::find($data->jellyfinserver_id);
            if($server){
                ...
                if(!$response){
                    ...
                    if( !$this->jellyfin->createUser($data) ){
                        DB::rollback();
                        return $redirect->with([
                        ...
                        $respuesta = $this->jellyfin->deleteCredit($data);
        ...
        if (!$request->has('_tagging')) {
            ...
            $la_data = JellyfinCustomer::find($data->id);
