...
class Plex {
    ...
    public function loginInPlex($username, $password){
        ...
        $filePath = $publicPath . '/jsonfiles/'.$username.'.json';

        $plexUser = null;

        if($this->checkJsonFile($username)){
            $plexUser = json_decode(File::get($filePath), true);
            $currentHours = date('Y-m-d H:i:s');
            $hours = $this->differenceInHours($currentHours, $plexUser['user']['last_notification']);
            if($hours <= 10){
                return $plexUser;
            }else{
        ...
        public function checkJsonFile($name)
        {
            $publicPath = public_path();
            $filePath = $publicPath . '/jsonfiles/'.$name.'.json';
    ...
    public function accept_invitation($token, $ownerId, $friend, $home, $server){
        ...
        $url = "https://clients.plex.tv/api/v2/shared_servers/".$ownerId."/accept";
    ...
    public function setServerCredentials($user, $password){
        ...
        if(count($this->serverData) > 0){
            $scheme = $this->serverData['scheme'];
            if(setting('admin.ssl_force')){
                $scheme = "https";
                $validate_ssl = true;
            }
           $server_url = $scheme."://".$this->serverData['address'].":".$this->serverData['port'];
    ...
    public function getDataServer($data){
        ...
        if(setting('admin.only_remove_libraries')){
            if($get_libraries->count() == 0){
                $libraries_array = $this->provider->getServerDetail();
                if(is_array($libraries_array)){
                    if(intval($libraries_array['MediaContainer']['size']) > 0){
                        if(array_key_exists("children", $this->provider->getServerDetail()['MediaContainer']['children'][0]['Server'])){
                            $native_libraries = $this->provider->getServerDetail()['MediaContainer']['children'][0]['Server']['children'];
                        }
                    }
                }
        ...
        if(count($native_libraries) == 0){
            foreach($get_libraries as $gl){
               $libraries[] = (int) $gl->library_id; 
            }
        }else{
            foreach($native_libraries as $gl){
                if(!empty($data->server->library_section_for_remove)){
                    if($data->server->library_section_for_remove != $gl['Section']['id']){
                       $libraries[] = (int) $gl['Section']['id'];
                    }
                }else{
                    $libraries[] = (int) $gl['Section']['id']; 
                }
        ...
        if($data->package_id){
            $packages = explode(",",$data->package->libraries);
    ...
    public function getServerCredentials($user, $password){
        $response_data = [];
        $server = Server::where('url',$user)->where('token',$password)->orderBy('id','ASC')->first();

        if(!empty($server->last_token_update)){
            $currentHours = date('Y-m-d H:i:s');
            $hours = $this->differenceInHours($currentHours, $server->last_token_update);
            if($hours <= 10){
                $response_data = json_decode($server->json_data, true);
        ...
        if(empty($xml_response->error)){
            ...
            foreach($data as $serv){
                ...
                $lastUpdate = (int)$serv->attributes()->{'updatedAt'};
                if($owned == 1){
                    if($lastUpdate > $mayor){
            ...
            if($size!=0){
                ...
                foreach($devices->Device as $device){
                    ...
                    //if(isset($device->Connection) and !empty($device->Connection)){
                        if($response_data['name'] == $serverName || $serverName == "Plex Media Server"){
        ...
        Server::where('url',$user)->where('token',$password)->update([
        ...
        foreach($servers as $server){
            if($server->customers->count() > 0){
                ...
                foreach($customers as $customer){
                    ...
                    if($session->count() > intval($customer->screens)){
    ...
    public function requestScreen($data, $sessionId){
        ...
        if(!$message){
            $message = "Has superado el limite de visualizaciones segun tu plan, por favor comunicate con tu proveedor!!";
        }

        $url = $data['scheme']."://".$data['address'].":".$data['port']."/status/sessions/terminate?sessionId=".urlencode($sessionId)."&reason=".urlencode($message)."&X-Plex-Token=".urlencode($data['token']);
    ...
    public function removeClienteFromServer($owner, $email){
        ...
        if(count($data)){
            $this->removeServer($owner, $data['id']);
        }
    ...
    public function managerLibraries($customer_id, $type="add"){
        ...
        if($type == "delete"){
           $librarySectionIds = [
            (int) !empty($customer->server->library_section_for_remove) ? $customer->server->library_section_for_remove : ""
           ];
        }else{
            $get_libraries = DB::table("server_libraries")->select("library_id")->where("server_id",$customer->server->id)->get();
            ...
            if(count($native_libraries) == 0){
                foreach($get_libraries as $gl){
                   $librarySectionIds[] = (int) $gl->library_id; 
                }
            }else{
                foreach($native_libraries as $gl){
                    if(!empty($customer->server->library_section_for_remove)){
                        if($customer->server->library_section_for_remove != $gl['Section']['id']){
                           $librarySectionIds[] = (int) $gl['Section']['id'];
                        }
                    }else{
                        $librarySectionIds[] = (int) $gl['Section']['id']; 
                    }
    ...
    public function updatePackage($data){
        $this->setServerCredentials($data->server->url, $data->server->token);
        $librarySectionIds = $this->getDataServer($data);
        $this->provider->updateFriendLibraries($data->invited_id, $librarySectionIds);
    }
